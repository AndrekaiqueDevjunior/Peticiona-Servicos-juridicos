...
const serviceCodeForType = (tipoPeticao: string) => {
  if (!tipoPeticao) return "solicitacao-juridica";
  if (tipoPeticao === "Contestação") return "contestacao";
  if (
    tipoPeticao.includes("Apelação") ||
    tipoPeticao.includes("Agravo") ||
    tipoPeticao.includes("Recurso")
  ) {
    return "recurso-apelacao";
  }
  if (
    tipoPeticao.includes("inicial") ||
    tipoPeticao.includes("Mandado de segurança") ||
    tipoPeticao.includes("Cumprimento de sentença")
  ) {
    return "peticao-inicial";
  }
  return "solicitacao-juridica";
};

const serviceGroupForType = (tipoPeticao: string): "A" | "B" =>
  serviceCodeForType(tipoPeticao) === "recurso-apelacao" ? "B" : "A";
...
export const NewRequestDialog = ({ open, onOpenChange }: NewRequestDialogProps) => {
  ...
  const serviceCode = serviceCodeForType(tipoPeticao);
  const tipoReconhecido = Boolean(areaDireito);
  const prazoCalc = tipoReconhecido
    ? calcularPrazo({ tipo: "avulso", grupo: serviceGroupForType(tipoPeticao) })
    : null;
  const { data: preview, isFetching: previewLoading } = useQuery({
    queryKey: ["client-order-preview", areaDireito, tipoPeticao, serviceCode],
    queryFn: () =>
      api.clientArea.previewOrder({
        area_direito: areaDireito,
        tipo_peticao: tipoPeticao || areaDireito,
        service_code: serviceCode,
      }),
    enabled: open && tipoReconhecido,
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 2. Criar petição com dados jurídicos; o backend cria o pedido administrativo vinculado.
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <section className="space-y-4">
            ...
            <div className="rounded-lg border border-border bg-muted/30 p-4">
              {!tipoReconhecido ? (
                <p className="text-sm text-muted-foreground">
                  Selecione a área do Direito para ver o serviço e o valor aplicado.
                </p>
              ) : (
                <>
                  <div className="space-y-1.5 border-b border-border pb-3">
                    <div className="flex items-center justify-between text-sm">
                      ...
                      <span className="font-medium text-foreground">
                        {(preview?.items[0]?.title ?? tipoPeticao) || areaDireito}
                      </span>
                    </div>
                    <div className="flex items-center justify-between text-sm">
                      <span className="text-muted-foreground">Código</span>
                      <span className="font-medium text-foreground">
                        {preview?.items[0]?.code ?? serviceCode}
                      </span>
                    </div>
                    {prazoCalc && (
                      <>
                  ...
                  <div className="mt-3 flex items-center justify-between">
                    ...
                    <span className="font-display text-2xl font-semibold text-primary">
                      {previewLoading ? "Calculando..." : preview?.total_brl ?? "R$ 0,00"}
                    </span>
                  </div>
                  ...
                  <div className="mt-4 space-y-2 border-t border-border pt-3 text-sm">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="font-medium">Catálogo do backend</span>
                    </div>
                    <div className="flex items-center justify-between">
          ...
          <DialogFooter>
            ...
            <Button
              type="submit"
              disabled={submitting || !tipoReconhecido || previewLoading}
