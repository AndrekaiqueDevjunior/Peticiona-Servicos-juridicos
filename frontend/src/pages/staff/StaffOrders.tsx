      ...
      <div className="flex items-end justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Todos os pedidos cadastrados na plataforma.
          </p>
        </div>
        ...
        <div className="space-y-6">
          ...
          {order.petition && (
            <div className="space-y-4 border-t border-border pt-4">
              <div>
                <p className="text-xs font-semibold text-foreground">Detalhes do formulário</p>
                <p className="text-xs text-muted-foreground">Petição {order.petition.reference}</p>
              </div>
              <div className="grid gap-4 sm:grid-cols-2">
                <Info label="Área do Direito" value={order.petition.area_direito} />
                <Info label="Tipo de petição" value={order.petition.tipo_peticao ?? "—"} />
                <Info label="Número do processo" value={order.petition.numero_processo ?? "—"} />
                <Info label="Data de publicação" value={order.petition.data_publicacao ?? "—"} />
                <Info label="Justiça gratuita" value={order.petition.justica_gratuita ? "Sim" : "Não"} />
                <Info label="Tutela de urgência" value={order.petition.tutela_urgencia ? "Sim" : "Não"} />
              </div>
              {order.petition.resumo_caso && (
                <Info label="Resumo do caso" value={order.petition.resumo_caso} />
              )}
              {order.petition.detalhes && (
                <Info label="Detalhes" value={order.petition.detalhes} />
              )}
              {order.petition.partes && order.petition.partes.length > 0 && (
                <div className="space-y-2">
                  <p className="text-xs font-semibold text-foreground">Partes</p>
                  <div className="divide-y divide-border rounded-md border border-border">
                    {order.petition.partes.map((parte, index) => (
                      <div
              ...
              {order.petition.documents && order.petition.documents.length > 0 && (
                <div className="space-y-2">
                  <p className="text-xs font-semibold text-foreground">Documentos</p>
                  <div className="divide-y divide-border rounded-md border border-border">
                    {order.petition.documents.map((document) => (
