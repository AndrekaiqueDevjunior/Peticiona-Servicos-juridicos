...
type PlanoId = (typeof PLANOS_INFO)[number]["id"];
type PackageId = PlanoId | CreditoAvulsoTipo;
...
export const BuyCreditsDialog = ({ open, onOpenChange }: BuyCreditsDialogProps) => {
  ...
  useEffect(() => {
    ...
    setForm((current) => ({
      ...
      cardHolder: current.cardHolder || profile.fullName,
    }));
  ...
  }, [open, profile.cpf, profile.phone, profile.fullName]);
      ...
      <TabsContent value="planos" className="mt-4 grid gap-4 md:grid-cols-3">
        {PLANOS_INFO.map((p) => (
          <div key={p.id} className="flex flex-col rounded-lg border border-border bg-card p-5">
            <h4 className="font-display text-lg font-semibold text-foreground">{p.nome}</h4>
            ...
            <ul className="mt-3 flex-1 space-y-1.5 text-sm text-muted-foreground">
              ...
              <li className="flex items-start gap-2">
                <Check className="mt-0.5 h-4 w-4 shrink-0 text-accent" />
                {formatBRL(p.porPedido)} por pedido
              </li>
            </ul>
            <Button
              type="button"
      ...
      <TabsContent value="avulsos" className="mt-4 grid gap-4 sm:grid-cols-2">
        {AVULSOS_INFO.map((a) => (
          <div key={a.id} className="flex flex-col rounded-lg border border-border bg-card p-4">
            ...
            <p className="mt-1 flex-1 text-xs text-muted-foreground">{a.descricao}</p>
            <Button
              type="button"
              variant="outline"
              className="mt-3"
