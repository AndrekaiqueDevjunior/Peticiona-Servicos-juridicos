...
export const criarPedido = (input: CriarPedidoInput): Pedido => {
  ...
  // Notifica equipe por e-mail (contato global da plataforma).
  void notifyOrderEvent({
    event: "pedido_criado",
    pedidoNumero: novo.numero,
    pedidoReference: novo.reference,
    detalhes: `${novo.tipoPeticao || novo.areaDireito} • ${novo.modalidadeLabel}`,
  });
...
export const adicionarComentario = (
  ...
  let pedidoRef: { numero: number; reference: string } | null = null;
  setState((s) => ({
    ...s,
    pedidos: s.pedidos.map((p) =>
      p.id === pedidoId
        ? (() => {
            pedidoRef = { numero: p.numero, reference: p.reference };
            return {
              ...p,
              comentarios: [
                ...p.comentarios,
  ...
  // Comentários internos da equipe não devem disparar e-mail externo.
  if (!interno && pedidoRef) {
    void notifyOrderEvent({
      event: "comentario_publicado",
      pedidoNumero: pedidoRef.numero,
      pedidoReference: pedidoRef.reference,
      autor: autorNome,
      detalhes: texto,
    });
  }
...
export const atualizarStatus = (pedidoId: string, status: PedidoStatus) => {
  setState((s) => ({
    ...
    pedidos: s.pedidos.map((p) => {
      ...
      const jaConcluido = p.status === "concluido";
      const ficouConcluido = status === "concluido" && !jaConcluido;
      if (ficouConcluido) {
        notify(
          "pedido_concluido",
          `Pedido ${p.reference} concluído`,
          `Confira em "Meus pedidos".`,
        );
      }
      return {
        ...p,
