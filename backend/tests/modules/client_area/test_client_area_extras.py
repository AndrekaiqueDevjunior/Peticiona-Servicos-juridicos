...
class TestCreateClientOrderWithBalance:
    def test_create_order_debits_credit_and_balance_endpoint_reflects(
        ...
        # Sistema opera em unidades: 1 crédito = 1 pedido
        credit_ledger.credit(
            client_user, amount=3,
            source="checkout", description="Compra de créditos",
            idempotency_key="extra-test-in-1",
        ...
        assert body["balances"]["common"] == 2  # 3 - 1 = 2
        assert any(
            movement["type"] == "out"
            and movement["source"] == "client_order"
            and movement["amount"] == 1
    ...
    def test_create_order_without_enough_balance_is_400_and_does_not_debit(
        ...
        # Nenhum crédito creditado — saldo zero
        response = api_client.post(
            "/api/client-area/orders",
            json={"service_code": "servico_peticao", "tipo_peticao": "Petição inicial comum"},
        )
        ...
        assert response.status_code == 400
        assert "insuficiente" in response.get_json()["message"].lower()
        balance = api_client.get("/api/me/balance").get_json()
        assert balance["balances"]["common"] == 0
